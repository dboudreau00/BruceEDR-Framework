...
///
/// ShieldHost had zero tests: it needs an ETW session and Administrator rights to start, so
...
/// </summary>
public sealed class ShieldHostTests : IDisposable
{
    private readonly List<ShieldHost> _hosts = new();
        ...
        bool autoKill = false)
    {
        ...
        var host = new ShieldHost(engine, response, new NullScanner(), log, new ShieldHostOptions
        {
            AutoKill = autoKill,
            StartMonitors = false,
