...
internal static class TelemetrySchemaFixtures
{
    ...
    internal static ShieldEvent Rich() => new()
    {
        Level = "QUARANTINE",
        Category = "detection",
    ...
    internal static ShieldEvent Empty() => new()
    {
        Level = "",
        Category = "",
        TimeUtc = default,
    ...
    /// </summary>
    internal static ShieldEvent Nulls() => new()
    {
        Level = null!,
        Category = null!,
        Process = null!,
    ...
    internal static ShieldEvent DirtyLists() => new()
    {
        Level = "WARN",
        Category = "detection",
        Reasons = new[] { "ok", null!, "  ", "" },
...
public class TelemetryFormatterContractTests
{
    ...
    public void Never_Throws_On_Default_Event(IEventFormatter f)
        => Assert.False(string.IsNullOrEmpty(f.Format(new ShieldEvent())));
...
{
    private static JsonElement Ecs(ShieldEvent e)
    ...
    public void Emits_Core_Envelope_Fields()
    {
        ...
        Assert.Equal("processshield.detection", TelemetrySchemaFixtures.At(root, "event", "dataset").GetString());
        Assert.Equal("processshield", TelemetrySchemaFixtures.At(root, "event", "module").GetString());
        Assert.Equal("ProcessShield", TelemetrySchemaFixtures.At(root, "event", "provider").GetString());
    ...
    {
        var root = Ecs(new ShieldEvent { Level = level, TimeUtc = default });
    ...
    {
        var root = Ecs(new ShieldEvent { Category = category, Level = level, TimeUtc = default });
    ...
    {
        var root = Ecs(new ShieldEvent { Category = "detection", Level = "QUARANTINE", TimeUtc = default });
    ...
    {
        var root = Ecs(new ShieldEvent { Category = "response", Level = "ACTION", TimeUtc = default });
    ...
    {
        var root = Ecs(new ShieldEvent { Trigger = "Credential Store Read!", TimeUtc = default });
    ...
    {
        var root = Ecs(new ShieldEvent { Techniques = new[] { "T9999.999" }, TimeUtc = default });
    ...
    public void Bare_Ipv6_Without_Port_Is_Not_Mis_Split()
    {
        ...
        var root = Ecs(new ShieldEvent { RemoteEndpoints = new[] { "2606:4700::1111" }, TimeUtc = default });
    ...
    {
        var root = Ecs(new ShieldEvent { RemoteEndpoints = new[] { endpoint }, TimeUtc = default });
        Assert.False(TelemetrySchemaFixtures.Has(root, "destination", "port"));
    ...
    public void Multiple_Endpoints_Do_Not_Guess_A_Destination()
    {
        ...
        Assert.Equal(new[] { "1.2.3.4:80", "5.6.7.8:443" },
            TelemetrySchemaFixtures.Strings(TelemetrySchemaFixtures.At(root, "processshield", "remote_endpoints")));
    }
    ...
    {
        var root = Ecs(TelemetrySchemaFixtures.Rich());
        var ps = TelemetrySchemaFixtures.At(root, "processshield");
    ...
    public void Unspecified_Kind_Timestamp_Is_Treated_As_Utc_Not_Shifted()
    {
        ...
        var e = new ShieldEvent { TimeUtc = new DateTime(2026, 3, 4, 5, 6, 7, 890, DateTimeKind.Unspecified) };
...
{
    private static JsonElement Ocsf(ShieldEvent e)
    ...
    public void Metadata_Names_The_Product_And_Schema_Version()
    {
        ...
        Assert.Equal("ProcessShield", TelemetrySchemaFixtures.At(root, "metadata", "product", "name").GetString());
        Assert.Equal("ProcessShield", TelemetrySchemaFixtures.At(root, "metadata", "product", "vendor_name").GetString());
    ...
    public void Severity_Id_Follows_The_Ocsf_Scale(string level, int expected)
        => Assert.Equal(expected, Ocsf(new ShieldEvent { Level = level, TimeUtc = default })
    ...
    public void Status_Id_Distinguishes_Findings_From_Chatter(string category, string level, int expected)
        => Assert.Equal(expected, Ocsf(new ShieldEvent { Category = category, Level = level, TimeUtc = default })
    ...
    {
        var root = Ocsf(new ShieldEvent { Level = "WARN", Pid = 3, TimeUtc = default });
...
{
    private static string Cef(ShieldEvent e) => new CefFormatter(TelemetrySchemaFixtures.Host).Format(e);
    ...
    public void Header_Has_Seven_Pipe_Delimited_Fields()
    {
        ...
        Assert.Equal("ProcessShield", f[1]);
        Assert.Equal("ProcessShield", f[2]);
    ...
    public void Header_Never_Contains_A_Line_Break()
    {
        ...
        var line = Cef(new ShieldEvent { Trigger = "one\ntwo\r\nthree", TimeUtc = default });
    ...
    public void Extension_Value_Escaping_Matrix(string raw, string expected)
    {
        ...
        string line = Cef(new ShieldEvent { IncidentId = raw, TimeUtc = default });
    ...
    {
        string line = Cef(new ShieldEvent
        {
            RemoteEndpoints = new[] { "1.2.3.4:80", "5.6.7.8:443" },
            TimeUtc = default
        });
    ...
    public void Empty_Event_Still_Produces_A_Valid_Header()
    {
        ...
        Assert.Equal("CEF:0", f[0]);
        Assert.Equal("processshield", f[4]);   // signature id fallback
...
public class TelemetryFormattingSinkTests : IDisposable
{
    private readonly string _dir = Path.Combine(
        Path.GetTempPath(), "shield_fmt_" + Guid.NewGuid().ToString("N"));
    ...
    public void Creates_The_Parent_Directory()
    {
        ...
        sink.Emit(new ShieldEvent { Level = "INFO" });
    ...
    private sealed class ThrowingFormatter : IEventFormatter
    {
        ...
        public string Format(ShieldEvent e) => throw new InvalidOperationException("boom");
    }

    private sealed class MultilineFormatter : IEventFormatter
    {
        public string Name => "multiline";
        public string Format(ShieldEvent e) => "a\nb\r\nc";
    }
}
...
{
    private static ShieldMetrics New(out ManualClock clock)
    {
        clock = new ManualClock(new DateTime(2026, 5, 1, 12, 0, 0, DateTimeKind.Utc));
        return new ShieldMetrics(clock);
    }
    ...
    public void Reservoir_Slides_So_Percentiles_Reflect_Recent_Behaviour()
    {
        ...
        for (int i = 0; i < ShieldMetrics.ReservoirSize; i++) m.ObserveDetectionLatency(TimeSpan.FromMilliseconds(100));
        var s = m.Snapshot();
        Assert.Equal(100, s.LatencyP50Ms);
        Assert.Equal(4000 + ShieldMetrics.ReservoirSize, s.LatencySamples);
    }
    ...
    public void Prometheus_Text_Declares_Help_And_Type_For_Every_Metric()
    {
        ...
        foreach (var name in new[]
                 {
                     "processshield_signals_total", "processshield_signals_dropped_total",
                     "processshield_detections_total", "processshield_responses_total",
                     "processshield_detection_latency_ms", "processshield_queue_depth"
                 })
        {
    ...
    public void Prometheus_Renders_The_Expected_Counter_Lines()
    {
        ...
        Assert.Contains("\nprocessshield_signals_total 7\n", "\n" + text);
        Assert.Contains("\nprocessshield_signals_dropped_total 2\n", "\n" + text);
        Assert.Contains("processshield_detections_total{level=\"warn\"} 1\n", text);
        Assert.Contains("processshield_detections_total{level=\"quarantine\"} 2\n", text);
        Assert.Contains("processshield_responses_total{result=\"ok\"} 1\n", text);
        Assert.Contains("processshield_responses_total{result=\"failed\"} 1\n", text);
    }
    ...
    public void Prometheus_Summary_Has_Quantiles_Count_And_Sum()
    {
        ...
        string text = m.ToPrometheusText();
        Assert.Contains("# TYPE processshield_detection_latency_ms summary\n", text);
        Assert.Contains("processshield_detection_latency_ms{quantile=\"0.5\"} 5\n", text);
        Assert.Contains("processshield_detection_latency_ms{quantile=\"0.95\"} 10\n", text);
        Assert.Contains("processshield_detection_latency_ms{quantile=\"0.99\"} 10\n", text);
        Assert.Contains("processshield_detection_latency_ms_count 10\n", text);
        Assert.Contains("processshield_detection_latency_ms_sum 55\n", text);
    }
    ...
    public void Gauge_Names_Are_Mapped_Into_The_Prometheus_Grammar()
    {
        ...
        string text = m.ToPrometheusText();
        Assert.Contains("processshield_queue_depth_pending 4\n", text);
    }
    ...
    {
        var m = New(out _);
        m.SetGauge("processshield_custom", 1);
        Assert.Contains("\nprocessshield_custom 1\n", "\n" + m.ToPrometheusText());
        Assert.DoesNotContain("processshield_processshield_custom", m.ToPrometheusText());
    }
    ...
    public void Colliding_Gauge_Names_Do_Not_Emit_Duplicate_Metric_Lines()
    {
        ...
        m.SetGauge("a-b", 2);
        string text = m.ToPrometheusText();
        int typeLines = text.Split('\n').Count(l => l == "# TYPE processshield_a_b gauge");
    ...
    public void A_Gauge_Cannot_Shadow_A_Built_In_Metric()
    {
        ...
        string text = m.ToPrometheusText();
        Assert.Equal(1, text.Split('\n').Count(l => l == "# TYPE processshield_signals_total counter"));
        Assert.Contains("processshield_signals_total 7\n", text);
        Assert.DoesNotContain("processshield_signals_total 99", text);
        Assert.DoesNotContain("processshield_detection_latency_ms_count 99", text);
    }
    ...
    public void Help_Text_Escapes_The_Raw_Gauge_Name()
    {
        ...
        m.SetGauge(@"weird\name", 1);
        string text = m.ToPrometheusText();
        Assert.Contains(@"Gauge 'weird\\name' reported by ProcessShield.", text);
        Assert.Contains("processshield_weird_name 1\n", text);
    }
    ...
    public void Prometheus_Formats_Non_Finite_And_Fractional_Values(double value, string expected)
    {
        var m = New(out _);
        m.SetGauge("v", value);
        Assert.Contains("processshield_v " + expected + "\n", m.ToPrometheusText());
    }
    ...
    public void Prometheus_Numbers_Are_Invariant_Not_Locale_Formatted()
    {
        ...
        try
        {
            ...
            Assert.Contains("processshield_fraction 1.5\n", m.ToPrometheusText());
        }
