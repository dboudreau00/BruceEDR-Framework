...
public class ResponseV2NetworkIsolationTests
{
    ...
    public void Isolate_Commands_Have_The_Expected_Shape()
    {
        ...
        Assert.Equal(new[]
        {
            ...
            "name=BruceEDR Isolation Allow Out",
            "dir=out", "action=allow", "remoteip=10.0.0.5", "enable=yes", "profile=any"
        }, cmds[0]);
        Assert.Equal(new[]
        {
            "advfirewall", "firewall", "add", "rule",
            "name=BruceEDR Isolation Allow In",
    ...
    public void Release_Restores_The_Policy_First_Then_Removes_The_Rules()
    {
        ...
        Assert.Equal(new[] { "advfirewall", "firewall", "delete", "rule", "name=BruceEDR Isolation Allow Out" }, cmds[1]);
        Assert.Equal(new[] { "advfirewall", "firewall", "delete", "rule", "name=BruceEDR Isolation Allow In" }, cmds[2]);
    ...
    [Fact]
    public void Every_Rule_Name_Carries_The_BruceEDR_Prefix()
    {
        var all = NetworkIsolation.BuildIsolateCommands(new[] { "10.0.0.5" })
            .Concat(NetworkIsolation.BuildReleaseCommands())
            .SelectMany(c => c)
...
public class ResponseV2TriageCollectorTests
{
    ...
    public void Collect_Produces_A_Readable_Zip_For_The_Current_Process()
    {
        ...
        var stagingBefore = new HashSet<string>(Directory.GetDirectories(temp, "BruceEDR-triage-*"));
        ...
        var leftovers = Directory.GetDirectories(temp, "BruceEDR-triage-*")
