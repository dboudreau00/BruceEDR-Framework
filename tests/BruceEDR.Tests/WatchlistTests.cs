...
public sealed class WatchlistTests
{
    ...
    private static DetectionEngine Engine(Watchlist list, bool trusted = false,
        ...
        => new(new EngineOptions(),
               _ => trusted,
               // The alert policy rides on the compiled list, so it can hot-reload with it.
               new EngineDependencies
               {
                   Watchlist = alertOnEveryHit ? list : Rebuild(list, alertOnEveryHit: false),
                   ImageHash = hash,
               });

    /// <summary>Recompiles a list with a different alert policy, for the tests that vary it.</summary>
    private static Watchlist Rebuild(Watchlist list, bool alertOnEveryHit)
        => Watchlist.Compile(
            list.Entries.Select(e => new WatchlistEntryConfig
            {
                Value = e.RawPattern,
                Match = e.Kind.ToString().ToLowerInvariant() switch
                {
                    "commandline" => "cmdline",
                    var other => other,
                },
    ...
    public void Quarantine_entry_contains_on_sight_with_no_other_evidence()
    {
        ...
        // Containment here is POLICY, not accumulated evidence, so it contributes no score
        // at all -- the forced verdict carries it. Points would be invisible escalation.
